...
function NewCustomers() {
  ...
  const updateBalance = () => {
    ...
    setCookie("new-customers-last-update", currentDate.toISOString()); // Store the exact timestamp
  };
    ...
    const fetchBalance = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/merchants/metrics/customers?period=${period}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          setPercentage(data.newCustomersChange);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const lastUpdate = getCookie("new-customers-last-update");
      if (lastUpdate) {
        setRelativeTime(
