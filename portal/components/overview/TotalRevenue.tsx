...
function TotalRevenue() {
  ...
  const updateBalance = () => {
    ...
    setCookie("revenue-last-update", currentDate.toISOString()); // Store the exact timestamp
  };
    ...
    const fetchBalance = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/merchants/metrics/revenue?period=${period}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          setPercentage(data.revenueChangePercentage)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const lastUpdate = getCookie("revenue-last-update");
      if (lastUpdate) {
        setRelativeTime(
